...
export const ChatBody: React.FC<ChatBodyProps> = ({
  ...
  return <div className="relative flex w-full flex-col overflow-hidden flex-1 pt-[72px] pb-4 px-6 max-md:max-w-full max-md:px-5 justify-end">
      <div className="self-center relative z-0 w-[750px] max-w-[750px] flex-1 max-md:max-w-full">
        ...
        <div className="z-0 w-full mt-8 max-md:max-w-full">
          {messages.map(message => <div key={message.id} className="mt-5 first:mt-0">
              <ChatMessage avatar={message.avatar} content={message.content} isUser={message.isUser} />
            </div>)}
        </div>
